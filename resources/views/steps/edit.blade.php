            ...
            <div class="card border-0 shadow-lg rounded-4">
                <div class="card-header bg-white py-4 border-bottom-0">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h4 class="mb-0 fw-bold">Edit Step</h4>
                            <p class="text-muted mb-0">Updating properties for: <span class="text-primary fw-bold">{{ $step->name }}</span></p>
                        </div>
                    </div>
                </div>
                <div class="card-body px-4 pb-4">
                    <form method="POST" action="{{ route('workflow.steps.update', $step) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-4">
                            <div class="col-md-12">
                                ...
                                <select name="process_id" id="process_id" class="form-select rounded-3" required>
                                    @foreach($processes as $id => $name)
                                        <option value="{{ $id }}" {{ $step->process_id == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-8">
                                ...
                                <input type="text" name="name" id="name" class="form-control rounded-3" value="{{ old('name', $step->name) }}" required>
                            </div>

                            <div class="col-md-4">
                                <label for="code" class="form-label fw-bold small text-uppercase text-secondary">Step Code</label>
                                <input type="text" name="code" id="code" class="form-control rounded-3" value="{{ old('code', $step->code) }}" required>
                            </div>
                            ...
                            <div class="col-md-8">
                                ...
                                <select name="workflow_action" id="workflow_action" class="form-select rounded-3" required>
                                    @foreach(config('workflow.workflow_actions') as $key => $data)
                                        <option value="{{ $key }}" {{ $step->workflow_action == $key ? 'selected' : '' }}>{{ is_array($data) ? $data['label'] : $data }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="number" name="stage_level" id="stage_level" class="form-control rounded-3" value="{{ old('stage_level', $step->stage_level) }}">
                            </div>

                            <div class="col-md-6">
                                <div class="form-check form-switch border p-2 rounded-3">
                                    <input class="form-check-input ms-0 me-2" type="checkbox" name="is_start" id="is_start" value="1" {{ $step->is_start ? 'checked' : '' }}>
                            ...
                            <div class="col-md-6">
                                <div class="form-check form-switch border p-2 rounded-3">
                                    <input class="form-check-input ms-0 me-2" type="checkbox" name="is_end" id="is_end" value="1" {{ $step->is_end ? 'checked' : '' }}>
                            ...
                            <div class="col-12">
                                ...
                                <div class="d-flex flex-wrap gap-3 p-3 bg-light rounded-3 border">
                                    @php $selectedRoles = $step->roles->pluck('id')->toArray(); @endphp
                                    @foreach($roles as $id => $name)
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="roles[]" value="{{ $id }}" id="role_{{ $id }}" {{ in_array($id, $selectedRoles) ? 'checked' : '' }}>
                            ...
                            <div class="col-12 d-flex justify-content-between align-items-center mt-5 border-top pt-4">
                                ...
                                <button type="submit" class="btn btn-primary px-5 py-2 rounded-pill fw-bold shadow-sm">
                                    Save Changes <i class="bi bi-check-lg ms-1 small"></i>
                                </button>
                            </div>
                        </div>
                    </form>
