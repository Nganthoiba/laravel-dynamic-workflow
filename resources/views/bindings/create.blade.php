{{--
    View: Create Binding Form
    
    Purpose:
    Renders a user-friendly form to set up a new workflow binding.
    Allows selection of the target workflow process, model type, lifecycle event, trigger mechanism, priority, and activation state.
    
    Usage:
    Returned by WorkflowBindingController@create, accessed via route('workflow-bindings.create').
            ...
            <div class="card border-0 shadow-lg rounded-4 overflow-hidden">
                <div class="card-header bg-white py-4 px-4 border-bottom-0">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h4 class="mb-0 fw-bold">Create New Binding</h4>
                            <p class="text-muted mb-0 small">Bind a workflow process to start automatically on specific model events</p>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <select name="model_type" id="model_type" class="form-select form-select-sm rounded-3 @error('model_type') is-invalid @enderror" required>
                                    ...
                                    @foreach($models as $key => $label)
                                        <option value="{{ $key }}" {{ old('model_type') == $key ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <select name="event_name" id="event_name" class="form-select form-select-sm rounded-3 @error('event_name') is-invalid @enderror" required>
                                    ...
                                    @foreach($events as $key => $label)
                                        <option value="{{ $key }}" {{ old('event_name') == $key ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-12 d-flex justify-content-between align-items-center mt-5 border-top pt-4">
                                ...
                                <button type="submit" class="btn btn-primary btn-sm px-5 py-2 rounded-pill fw-bold shadow-sm">
                                    Save Binding Rule <i class="bi bi-check-lg ms-1"></i>
                                </button>
                            </div>
                        </div>
                    </form>
