            ...
            <div class="col-md-8">
                <div class="card shadow-sm border-0">
                    <div
                        ...
                        <a href="{{ $readonly ?? false ? route('workflow.outbox') : route('workflow.inbox') }}"
                    ...
                    <div class="card-body pt-4">
                        <div class="mb-2">
                            <ul class="list-unstyled mb-0 d-flex flex-wrap gap-3 justify-content-around small">
                                <li class="mb-3">
                                ...
                                <li class="mb-3">
                                    <span class="fw-bold d-block text-muted small text-uppercase">Reference Type</span>
                                    <span class="text-primary fw-bold small">{{ class_basename($instance->reference_type) }}</span>
                                </li>
                    ...
                    @section('comment_field')
                        <div class="mb-4">
                            ...
                            @if(!($readonly ?? false))
                                <textarea name="comment" class="form-control" rows="4" placeholder="Enter your detailed remarks here... (optional)" ></textarea>
                            ...
                            if ($history->completed_at) {
                                ...
                                if (
                                    in_array($action, [
                                        ...
                                        'cancelled',
                                        'cancel',
                                    ]) /*|| in_array($refStatus, ['rejected', 'reject', 'cancelled', 'canceled'])*/
                                ) {
                            ...
                            <div class="timeline-content ps-4">
                                ...
                                @if ($history->completed_at)
                                    <div class="d-flex align-items-center mb-1">
                                        @if ($isRejectedOrCancelled)
                                            ...
                                            <small class="text-danger fw-semibold"
                                                style="font-size: 0.75rem;">{{ $history->action }}</small>
                                        @else
                                    ...
                                    <div class="small text-muted mb-1" style="font-size: 0.7rem;">
                                        ...
                                        {{ $history->completed_at->format('M d, Y h:i A') }}
                                    </div>
                                    @if ($history->comment)
                                        <div class="mt-2 p-2 bg-light rounded small border-start border-3 {{ $isRejectedOrCancelled ? 'border-danger' : 'border-success' }}"
                                ...
                                @else
                                    <div class="d-flex align-items-center mb-1">
                                        ...
                                        <small class="text-warning fw-semibold"
                                            style="font-size: 0.75rem;">Pending ...</small>
                                    </div>
                                    <div class="small text-muted" style="font-size: 0.7rem;">
                                        <i class="bi bi-people me-1"></i>
                                        @if ($history->step->roles->count() > 0)
                                            {{ $history->step->roles->pluck('display_name')->implode(', ') }}
                                        @else
                                            Anyone
                                        @endif
                                    </div>
